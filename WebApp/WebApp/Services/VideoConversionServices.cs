...
internal interface IVideoConversionJobStatusStore { bool HasActiveSource(string sourceId); void Seed(VideoConversionJob job); void Remove(string id); void Processing(string id); void Progress(string id, VideoConversionProgress progress); void Complete(string id, string outputId, long size); void Fail(string id, string message); void Skip(string id, string message); IReadOnlyList<VideoConversionStatus> GetAll(); }
...
{ private readonly ConcurrentDictionary<string, VideoConversionStatus> _jobs = new(); private readonly ConcurrentQueue<string> _order = new();
 public bool HasActiveSource(string sourceId) => _jobs.Values.Any(x => x.SourceId == sourceId && x.State is (VideoConversionJobState.Pending or VideoConversionJobState.Processing or VideoConversionJobState.Finalizing));
 public void Seed(VideoConversionJob j) { _jobs[j.JobId] = new(j.JobId, j.Source.Id, j.Source.Name, j.Action, VideoConversionJobState.Pending, j.Source.SizeBytes, QueuedAtUtc: DateTimeOffset.UtcNow, SourceDurationSeconds: j.Probe.Duration.TotalSeconds); _order.Enqueue(j.JobId); } public void Remove(string id) => _jobs.TryRemove(id, out _);
 public void Processing(string id) => Update(id, x => x with { State = VideoConversionJobState.Processing, StartedAtUtc = DateTimeOffset.UtcNow });
 public void Progress(string id, VideoConversionProgress progress) => Update(id, x => progress.IsFinalizing ? x with { State = VideoConversionJobState.Finalizing } : x with { State = VideoConversionJobState.Processing, ProcessedDurationSeconds = Math.Max(x.ProcessedDurationSeconds ?? 0, progress.ProcessedDurationSeconds ?? 0), Speed = progress.Speed ?? x.Speed });
 public void Complete(string id,string output,long size)=>Update(id,x=>x with{State=VideoConversionJobState.Completed,OutputItemId=output,OutputSizeBytes=size,ProcessedDurationSeconds=x.SourceDurationSeconds}); public void Fail(string id,string msg)=>Update(id,x=>x with{State=VideoConversionJobState.Failed,Diagnostic=msg}); public void Skip(string id,string msg)=>Update(id,x=>x with{State=VideoConversionJobState.Skipped,Diagnostic=msg}); private void Update(string id, Func<VideoConversionStatus,VideoConversionStatus> f)=>_jobs.AddOrUpdate(id,_=>throw new InvalidOperationException(),(_,x)=>f(x)); public IReadOnlyList<VideoConversionStatus> GetAll()=>_order.Select(x=>_jobs.GetValueOrDefault(x)).Where(x=>x is not null).Select(x=>x!).ToList(); }
...
internal interface IVideoConversionGenerator { Task<VideoConversionGenerationResult> GenerateAsync(VideoConversionJob job, Action<VideoConversionProgress>? reportProgress, CancellationToken token); }
internal sealed class FfmpegVideoConversionGenerator(IVideoConversionProbe probe, VideoConversionNamingService naming, IOptions<VideoConversionOptions> options) : IVideoConversionGenerator
{
 public async Task<VideoConversionGenerationResult> GenerateAsync(VideoConversionJob job, Action<VideoConversionProgress>? reportProgress, CancellationToken token)
 {
  ...
  try {
   ...
   var psi=new ProcessStartInfo { FileName="ffmpeg", UseShellExecute=false, RedirectStandardOutput=true, RedirectStandardError=true, CreateNoWindow=true }; foreach(var arg in BuildArguments(job.Source.PhysicalPath,temp,job.Action,options.Value.H264Crf)) psi.ArgumentList.Add(arg);
   using var p=Process.Start(psi); if(p is null)return new(false,false,Diagnostic:"ffmpeg failed to start."); var progressTask=ReadProgressAsync(p.StandardOutput,reportProgress,token); var errorTask=p.StandardError.ReadToEndAsync(token); await p.WaitForExitAsync(token); await progressTask; var error=await errorTask; if(p.ExitCode!=0)return new(false,false,Diagnostic:"ffmpeg could not convert this media."); reportProgress?.Invoke(new(null,null,true));
   var output=await probe.ProbeAsync(temp,token); if(output is null || !IsValidMp4(output) || output.HasSubtitles) return new(false,false,Diagnostic: output?.HasSubtitles == true ? "Embedded subtitles could not be safely published to MP4." : "Converted output failed validation.");
   ...
   File.Move(temp,dest); return new(true,false,dest,outputSize);
  } catch(OperationCanceledException){ return new(false,false,Diagnostic:"Conversion cancelled during shutdown."); } catch(Exception e) when(e is IOException or UnauthorizedAccessException or Win32Exception){ return new(false,false,Diagnostic:"Conversion could not publish output."); } finally { try { if(File.Exists(temp)) File.Delete(temp); } catch {} }
 }
...
internal sealed class VideoConversionBackgroundWorker(IVideoConversionJobQueue queue, IVideoConversionGenerator generator, IVideoConversionJobStatusStore statuses, IArchiveService archive, ILogger<VideoConversionBackgroundWorker> logger) : BackgroundService
{ protected override async Task ExecuteAsync(CancellationToken token) { while(!token.IsCancellationRequested) { VideoConversionJob job; try { job=await queue.DequeueAsync(token); } catch(OperationCanceledException){break;} statuses.Processing(job.JobId); try { var result=await generator.GenerateAsync(job,progress=>statuses.Progress(job.JobId,progress),token); if(result.Skipped) statuses.Skip(job.JobId,result.Diagnostic??"Skipped."); else if(result.Success && result.DestinationPath is not null) statuses.Complete(job.JobId,archive.ComputeItemId(job.Source.Category.Key,result.DestinationPath),result.OutputSizeBytes??0); else statuses.Fail(job.JobId,result.Diagnostic??"Conversion failed."); } catch(Exception e) when(e is not OperationCanceledException) { logger.LogWarning(e,"Video conversion failed for job {JobId}",job.JobId); statuses.Fail(job.JobId,"Unexpected conversion error."); } finally { queue.Complete(); } } } }
