...
| FR5–FR8 | `make archive-group` safely creates/verifies the group; `make archive-share USER=<existing-account>` grants that account group access, is idempotent, and rejects missing/nonexistent accounts without changing user ownership. |
...
1. On a disposable archive root, run `make archive-group`, then `make archive-share USER=<existing-admin>` twice; confirm the second run succeeds without changing user ownership.
...
3. Create a new nested folder/file as root with umask `0002` and confirm it inherits the `perenearchive` group and group-write access.
...
- `make archive-group` and `make archive-share USER=<account>` provide explicit, safe group-based archive editor access.
